...
"""
Step 55 - MT5 Tick Bridge Writer v1
...
Run example from backend_api_python folder:
...
def _spread_pips(symbol: str, bid: Optional[float], ask: Optional[float]) -> Optional[float]:
...
def build_payload(symbol: str, tick: Any, source: str = "mt5_python_tick") -> Dict[str, Any]:
    ...
    return {
        ...
        "spread_pips": _spread_pips(symbol, bid, ask),
        "note": "Read-only MT5 tick bridge. This file is for dashboard live reference only.",
    }


def run_bridge(symbol: str, interval: float, output_dir: Path, once: bool = False) -> int:
    try:
    ...
    symbol = _clean_symbol(symbol)
    output_path = output_dir / f"{symbol}_tick.json"
    ...
    try:
        if not mt5.symbol_select(symbol, True):
            print(f"MT5 symbol_select failed for {symbol}: {mt5.last_error()}", file=sys.stderr)
            return 4

        print(f"MT5 tick bridge started for {symbol}")
        ...
        while True:
            ...
            if tick is None:
                print(f"No tick returned for {symbol}: {mt5.last_error()}", file=sys.stderr)
            else:
                payload = build_payload(symbol, tick)
                _write_json_atomic(output_path, payload)
                print(f"{payload['written_at']} {symbol} bid={payload['bid']} ask={payload['ask']} spread={payload['spread_pips']}")
...
def main() -> int:
    ...
    parser.add_argument("--symbol", default="GBPJPY", help="Symbol to watch, for example GBPJPY")
    ...
    return run_bridge(args.symbol, args.interval, Path(args.output_dir), once=args.once)
